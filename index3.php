...
<html>
    ...
    <body id="body">
		...
		while($donnee = $reponse->fetch())
		{
            ...
            $persoArrayLenght++;
			echo "<span onclick=\"persoClick($persoArrayLenght)\">" . 'ip: '. '<mark>' . $donnee['ip'].'</mark>' .' | ipLocal: ' .'<mark>'  . $donnee['ipLocal'] . '</mark>' . ' | port: ' .'<mark>'. $donnee['port'] .'</mark>'. ' | computer name: ' . $donnee['pc_name'] . ' | userName: ' . $donnee['user_name']/* . ' | heure: ' . $heure2 . ':' . $minute*/ . " | version: " . $donnee['version'] . '</span>';
			echo '<br><br>';
		}
        ...
        <footer class="footer">
        <img class="logo" height="50px"  src="logo.png" alt="logo">
        <br>
